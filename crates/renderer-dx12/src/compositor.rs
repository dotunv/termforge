...
impl Compositor {
    ...
    pub fn build(mut ctx: Dx12Context, font_family: &str, font_size_pt: f32) -> Result<Self> {
    ...
    pub fn render_frame_with_chrome(
        ...
        unsafe {
            ...
            for (pane_idx, (session_id, px, py, pw, ph)) in pane_rects.iter().enumerate() {
                ...
                // Re-upload vertices only when the grid has new content, but
                // always issue the draw call using the last-valid vertex buffer.
                // Without this, every frame that clears the backbuffer but skips
                // the draw call (e.g. keypress before PTY echoes back) would
                // render a blank terminal area for one frame.
                if session.is_dirty() {
                    let vertices =
                        build_cell_quads(&session.grid, &self.atlas, *px, *py, *pw, *ph);
