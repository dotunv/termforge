...
impl TextInput {
    pub fn new(label: &'static str) -> Self {
        Self { label, value: String::new(), active: false }
    }
...
pub enum ManagerMode { List, NewHost }
        ...
        match self.mode {
            ManagerMode::List => {
                match c {
                    'n' | 'N' => {
                        ...
                        self.focus = 0;
                        for f in &mut self.fields { f.clear(); f.active = false; }
            ...
            ManagerMode::NewHost => {
                match c {
                    ...
                    '\x08' | '\x7f' => { self.fields[self.focus].pop(); } // backspace
                    '\x09' => self.next_field(), // Tab
                    _ if !c.is_control() => { self.fields[self.focus].push(c); }
                    _ => {}
                }
            }
        }
...
pub fn generate_ssh_manager_commands(
    ...
    cmds.push(UiCommand::FillRect { x: mx, y: my, w: modal_w, h: modal_h, color: hex(COL_PANEL) });
    // Border
    for (ox, oy, bw, bh) in [
    ...
    cmds.push(UiCommand::DrawText {
        ...
        fg: hex(COL_TEXT), bg: hex(COL_PANEL),
    });
    cmds.push(UiCommand::DrawText {
        x: mx + modal_w - 8.0 * cw - 20.0, y: my + 14.0,
        text: "[Ctrl+H] close".to_string(),
        fg: hex(COL_MUTED), bg: hex(COL_PANEL),
    });
    // Title underline
    cmds.push(UiCommand::FillRect {
        x: mx, y: my + ch + 20.0, w: modal_w, h: 1.0,
...
fn render_list(
    ...
    for (i, host) in state.hosts.iter().enumerate().skip(start).take(visible) {
        ...
        cmds.push(UiCommand::FillRect {
            x: mx + 1.0, y: iy, w: modal_w - 2.0, h: item_h,
            color: hex(item_bg),
        });
        if selected {
            cmds.push(UiCommand::FillRect {
                x: mx + 1.0, y: iy, w: 3.0, h: item_h,
                color: hex(COL_BLUE),
            });
        }
...
fn render_new_host(
    ...
    for (i, field) in state.fields.iter().enumerate() {
        ...
        // Label
        cmds.push(UiCommand::DrawText {
            x: mx + pad_x, y: fy + (field_h - ch) * 0.5,
        ...
        cmds.push(UiCommand::FillRect { x: box_x, y: fy, w: box_w, h: field_h, color: hex(COL_BG) });
        // Border
        for (ox, oy, bw, bh) in [
            (box_x, fy, box_w, 1.0),
            ...
            cmds.push(UiCommand::FillRect { x: ox, y: oy, w: bw, h: bh, color: hex(border_color) });
        }
        ...
        cmds.push(UiCommand::DrawText {
            x: box_x + 8.0, y: fy + (field_h - ch) * 0.5,
            text: display,
